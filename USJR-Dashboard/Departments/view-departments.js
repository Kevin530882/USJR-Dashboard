...
let editCollId = document.getElementById('editCollId');
    ...
    departments.forEach(department => {
        ...
        colleges.forEach(college => {
            if (college.collid == department.deptcollid) {
                ...
                col = college.collfullname;
            }
        })

        tr.innerHTML = `
            <td class="iamid">${department.deptid}</td>
            <td>${department.deptfullname}</td>
            <td>${department.deptshortname}</td>
            <td>${col}</td>
            <td>
                <button class="btn btn-light editdept">${edit}</button>
                <button class="btn btn-light deletedept">${del}</button>
            </td>
        `;
        tablebody.appendChild(tr);

        tr.querySelector('.editdept').addEventListener('click', () => {
            editCollId.innerHTML = '';
            currentRow = tr;
            editID.value = department.deptid;
            editFname.value = department.deptfullname;
            editSname.value = department.deptshortname;
            
            let collegeOption = document.createElement('option');
            collegeOption.value = colid;
            collegeOption.textContent = col;
            collegeOption.selected = true;
            editCollId.appendChild(collegeOption);

            editPanel.style.display = 'block';
            fetchCollegesAndPrograms(colid);
        });

        tr.querySelector('.deletedept').addEventListener('click', () => {
            currentRow = tr;
            const id = department.deptid;
...
edit.addEventListener('click', () => {
    if (currentRow) {
        ...
        const deptcollid = editCollId.value;
            ...
            if (result.isConfirmed) {
                ...
                .then(response => {
                    ...
                    if (response.data.status === 'success') {
                        ...
                        currentRow.querySelector('td:nth-child(4)').innerText = deptcollid;
...
function fetchCollegesAndPrograms(selectedCollegeId) {
    axios.get('../get-colleges-programs.php')
        .then(response => {
            console.log('Colleges data:', response.data);
            const colleges = response.data.colleges;
